...
Proof. intros n m H. induction n as [|n IH].
  - left. reflexivity.
  - simpl in H. apply and_exercise in H. 
  destruct H. right. apply H. Qed.

(** **** Exercise: 1 star, standard (or_commut) *)
Theorem or_commut : forall P Q : Prop,
  P \/ Q  -> Q \/ P.
Proof. intros P Q [PH | QH]. 
  right. apply PH.
  left. apply QH. Qed.
...
Theorem not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof. intros P notP Q p. apply notP in p. destruct p. Qed.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof. unfold not. intros P Q pq notq p. 
  apply notq. apply pq. apply p. Qed.

(** **** Exercise: 1 star, standard (not_both_true_and_false) *)
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof. intros P H. destruct H.
...
Proof. intros P. split. 
  - intros p. apply p. 
  - intros p. apply p. Qed.

Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof. intros P Q R [pq qp] [qr rq]. split.
  - intros p. apply qr. apply pq. apply p.
  - intros r. apply qp. apply rq. apply r. Qed.
...
Proof. intros P Q R. split.
  - intros [p|qr]. 
    + split.
      left. apply p.
      left. apply p.
    + destruct qr as [q r]. split.
      right. apply q.
      right. apply r.
  - intros [p_q p_r]. destruct p_q as [p|q].
    + left. apply p.
    + destruct p_r as [p|r].
      left. apply p. 
      right. split. apply q. apply r. Qed.
...
Proof. intros X P H [e E]. apply E. apply H. Qed.  
...
Proof. intros X P Q. split.
  - intros [e [E|E]].
    + left. exists e. apply E.
    + right. exists e. apply E.
  - intros [[e E]|[e E]].
    + exists e. left. apply E.
    + exists e. right. apply E. Qed.
...
Proof. intros A B f l y.
  ...
  - split. 
    + simpl. intros [H|H].
      ...
      apply IH in H. destruct H as [e [E1 E2]].
      exists e. split. apply E1. right. apply E2.
    + intros [e [E1 [E2|E2]]].
      left. rewrite E2. apply E1.
      right. apply IH. exists e. split. apply E1. apply E2. Qed.

(** **** Exercise: 2 stars, standard (In_app_iff) *)
Theorem In_app_iff : forall A l l' (a:A),
  In a (l++l') <-> In a l \/ In a l'.
Proof. intros A l l' a. induction l as [|h l IH].
  - split.
    + simpl. intros H. right. apply H.
    + intros [H|H]. inversion H. apply H.
  - simpl. rewrite IH. apply or_assoc. Qed. 
...
Theorem All_In :
  forall T (P : T -> Prop) (l : list T),
    (forall x, In x l -> P x) <->
    All P l.
Proof. intros T P l. induction l as [|h l IH].
  - split.
    + intros H. apply I.
    + intros H x H2. inversion H2.
  - split. 
    + intros H. split.
...
Proof. intros Podd Peven n H1 H2. unfold combine_odd_even. 
  destruct (odd n).
...
Proof.
  ...
  - destruct IH as [k IH'].
    ...
    + rewrite even_S. rewrite En'.
      exists k. simpl. rewrite IH'. reflexivity.
    + rewrite even_S. rewrite En'.
...
Proof. intros b1 b2. split.
  ...
  - intros [H|H]. 
    ...
    + rewrite H. destruct b1. reflexivity. reflexivity. Qed.
...
Proof.
  ...
  - (* -> direction *)
    ...
    + intros l2. destruct l2.
      * simpl. intros H'. discriminate H'.
      * simpl. intros H'. apply andb_true_iff in H' as [H_eqb H_rest].
        apply H in H_eqb. subst. f_equal. apply IH. assumption.
  - (* <- direction *)
    intros H'. subst.
...
Proof. intros X l. induction l as [|h l IH].
  - reflexivity.
  - intros l1 l2. simpl. 
...
Proof. intros X. apply functional_extensionality.
  intros l. unfold tr_rev.
  induction l as [|h l IH].
    - reflexivity.
    - simpl. rewrite <- IH. 
      replace (rev_append l [h]) with (rev_append l ([] ++ [h])).
      apply tr_rev_app. 
      reflexivity. Qed.
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof. intros P. unfold not. intros H. 
  assert (H2: P -> False).
    {intros p. apply (fun x => H (or_introl x)) in p. apply p.  }
  apply (fun x => H (or_intror x)) in H2. apply H2. 
Qed.
...
Proof. intros em X P H x. destruct (em (P x)).
  - apply H0. 
  - exfalso. apply H. exists x. apply H0. Qed.
