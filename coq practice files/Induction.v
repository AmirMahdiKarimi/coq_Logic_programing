...
Proof.
  ...
  - (* n = S n' *)
    rewrite <- plus_n_Sm. rewrite IHm'. reflexivity.  Qed.
...
Proof.
  ...
  - (* n = 0 *)
    simpl. rewrite -> add_0_r. reflexivity.
  - (* n = S n' *)
    simpl. rewrite <- plus_n_Sm. rewrite <- plus_n_Sm. simpl. rewrite IHm'. reflexivity.  Qed.
...
Theorem add_shuffle3 : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros. rewrite add_assoc. rewrite add_assoc. assert(H:n+m=m+n).
  { rewrite add_comm. reflexivity. }
...
Proof.
  ...
  - simpl.
   rewrite <- IHn. rewrite <- mult_n_Sm. rewrite add_comm. reflexivity. 
Qed.
...
Proof.
  intros. induction n.
  - simpl. reflexivity. 
  - simpl. rewrite <- plus_n_Sm. rewrite add_assoc. rewrite add_assoc.
...
Proof.
  ...
  - reflexivity.
  - simpl. 
  assert (H: forall b, bin_to_nat (incr b) = S (bin_to_nat b)).
  {
      intros b.
      induction b.
      - simpl. reflexivity.
